...
class Ball(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.top > SCREEN.bottom:
            self.update = self.start                    # ボールを初期状態に o
...
def main():
    ...
    Ball.paddle_sound   = pygame.mixer.Sound("system49.wav")    # パドルにボールが衝突した時の効果音取得
    Ball.block_sound    = pygame.mixer.Sound("system49.wav")    # ブロックにボールが衝突した時の効果音取得
    Ball.gameover_sound = pygame.mixer.Sound("system49.wav")    # ゲームオーバー時の効果音取得
    
    group = pygame.sprite.RenderUpdates()  # 描画処理用のスプライトグループ
